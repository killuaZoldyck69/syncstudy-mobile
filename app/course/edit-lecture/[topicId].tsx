...
interface SubTopicUI {
  id: string; // Real UUID from DB or temporary ID for new rows
  title: string;
}

export default function EditLectureScreen() {
  const { topicId, courseId } = useLocalSearchParams();
  ...
  useEffect(() => {
    const fetchTopicData = async () => {
      setIsLoading(true);
      // Fetch all topics and find the specific one we are editing
      const { data, error } = await courseService.getCourseTopics(
        courseId as string,
      );
      ...
      if (error) {
        ...
        const currentTopic = data.find((t: any) => t.id === topicId);
        if (currentTopic) {
          setTitle(currentTopic.title);
          setTermPhase(currentTopic.term_phase);
          setLectureDate(new Date(currentTopic.lecture_date));
          setDriveLink(currentTopic.note_drive_link || "");

          if (currentTopic.subTopics && currentTopic.subTopics.length > 0) {
            setSubTopics(
              currentTopic.subTopics.map((st: any) => ({
                id: st.id,
                title: st.title,
              })),
            );
          } else {
            setSubTopics([{ id: Date.now().toString(), title: "" }]); // Empty row fallback
          }
        }
      }
      setIsLoading(false);
    ...
    if (topicId && courseId) fetchTopicData();
  }, [topicId, courseId]);
  ...
  const handleUpdateLecture = async () => {
    ...
    const cleanedSubTopics = subTopics
      ...
      .map((st) => ({
        // Pass original ID if it's a real UUID (length check), otherwise undefined for new ones
        id: st.id.length > 20 ? st.id : undefined,
        title: st.title.trim(),
      }));

    setIsSubmitting(true);

    const { error } = await courseService.updateCourseTopic(
      courseId as string,
      topicId as string,
      {
        title,
    ...
    if (error) {
      Alert.alert("Failed to Update Lecture", error.message);
    } else {
